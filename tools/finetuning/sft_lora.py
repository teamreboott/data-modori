...
from data_loaders import SFTDataset, DataCollatorSFTDataset
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cmd_config_path", default='./config/mistral_7b_lora.yaml', type=str)
    ...
    if config.MODE == 'train':
        ...
        model, tokenizer, peft_config = load_peft_models(base_dir=config.BASE_DIR,
                                                         ...
                                                         max_len=config.TRAINER.model_max_length)

        # build dataset
        train_dataset = SFTDataset(tokenizer=tokenizer,
                                   data_config=config.DATASET,
                                   shuffle = True,
                                   is_hf=config.DATASET.is_hf,
        ...
        if os.path.isfile(os.path.join(config.BASE_DIR, config.DATASET['eval_path'])) or config.DATASET.is_hf:
            eval_dataset = SFTDataset(tokenizer=tokenizer,
                                      data_config=config.DATASET,
                                      is_hf=config.DATASET.is_hf,
                                      data_name=config.DATASET.dataset,
                                      train_or_test='val')
            ...
            eval_dataset = None
        data_collator = DataCollatorSFTDataset(tokenizer=tokenizer)
        ...
        # PEFT
        trainer = LoRATrainer(model=model,
                              data_collator=data_collator,
                              dataset_text_field='korean',
                              train_dataset=train_dataset,
                              eval_dataset=eval_dataset,
                              max_seq_length=config.TRAINER.model_max_length,
                              tokenizer=tokenizer,
                              args=train_args,
                              neftune_noise_alpha=5.0 if config.TRAINER.use_neft else None)
